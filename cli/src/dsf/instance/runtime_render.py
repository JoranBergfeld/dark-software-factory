...
def runtime_endpoint_env(outputs: Mapping[str, str]) -> dict[str, str]:
    """Map a product's Azure deployment outputs to the runtime ``AZURE_*`` env.

    Single source of truth (with :data:`_ENDPOINT_MAP`) for translating Bicep
    deployment outputs into the env the runtime and the operator CLI read. Blank
    or missing outputs are omitted. Used by the rendered ``.env.orchestrator`` and
    by ``dsf charter`` (manifest-derived settings).
    """
    return {var: outputs[key] for var, key in _ENDPOINT_MAP if outputs.get(key)}
...
def _render_env(product: str, outputs: dict[str, str]) -> str:
    endpoint_env = runtime_endpoint_env(outputs)
    lines = [
    ...
    lines.extend(f"{var}={endpoint_env.get(var, '')}" for var, _key in _ENDPOINT_MAP)
    return "\n".join(lines) + "\n"
