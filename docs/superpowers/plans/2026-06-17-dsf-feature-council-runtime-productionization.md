...
rendered — they are injected at runtime from Key Vault via the homelab service principal (ADR 0002: an Entra SP, not Managed Identity).
"""
...
def _render_env(product: str, outputs: dict[str, str]) -> str:
    lines = [
        ...
        "# injected at runtime (Key Vault via the homelab service principal, ADR 0002), never rendered here.",
        "DSF_MODE=azure",
...
them at runtime via Key Vault using the homelab service principal (ADR 0002). The bundle directory is
git-ignored.
