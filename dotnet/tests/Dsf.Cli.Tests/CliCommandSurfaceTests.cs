...
command <root> desc="Dark Software Factory — factory CLI (create product instances)"
  ...
  command new desc="create a new isolated product factory instance"
    ...
    option --github-installation-selection type=String arity=1..1 default="" choices=[all, selected] desc="owner DSF GitHub App installation selection"
  command onboard desc="onboard an existing application into a DSF phase"
    command decide desc="attach a proposal-only Feature Council to an existing application"
      command preview desc="preview the dedicated application boundary for an existing app (read-only)"
        option --product type=String arity=1..1 required desc="owner-scoped product key for the attached council"
        option --tenant type=String arity=1..1 desc="Azure tenant id (defaults to the Azure CLI identity)"
        option --subscription type=String arity=1..1 desc="Azure subscription id (defaults to the Azure CLI identity)"
        option --app-environment type=String arity=1..1 desc="named application environment to attach to (e.g. 'production')"
        option --application-resource type=String[] arity=1..100000 desc="resource id explicitly inside the application boundary (repeatable)"
        option --evidence-backend type=String[] arity=1..100000 desc="dedicated evidence backend resource id, selected separately (repeatable)"
        option --dedicated type=Boolean arity=0..1 default=False choices=[False, True] desc="declare this exact selection dedicated to the application"
        option --cross-owner-reviewed type=Boolean arity=0..1 default=False choices=[False, True] desc="confirm cross-owner exclusivity was reviewed for this exact selection"
        option --factory-environment type=String arity=1..1 default="dev" desc="factory environment moniker, distinct from the application environment"
        option --owner-appconfig-endpoint type=String arity=1..1 desc="owner App Configuration endpoint"
        option --owner-keyvault-uri type=String arity=1..1 desc="owner Key Vault URI"
        option --expect-fingerprint type=String arity=1..1 desc="reject the run unless the plan fingerprint matches this reviewed value"
  ...
  command sweep desc="sweep enabled source agents once (runtime)"
    option --dry-run type=Boolean arity=0..1 default=False choices=[False, True] desc="sweep the line but skip filing"
    option --product type=String arity=1..1 desc="resolve runtime env for this product"
  ...
  command serve-agent desc="serve a source agent over A2A (runtime)"
    ...
    option --port type=Int32? arity=1..1 default=8080 desc="bind port"
    option --product type=String arity=1..1 desc="resolve runtime env for this product"
  command poll-outcomes desc="poll human outcome labels and record audited learning data (runtime)"
    option --dry-run type=Boolean arity=0..1 default=False choices=[False, True] desc="poll and preview outcomes without recording them"
    option --live type=Boolean arity=0..1 default=False choices=[False, True] desc="poll and record outcomes for real (requires the manual confirmation gate)"
    option --product type=String arity=1..1 desc="resolve runtime env for this product"
