"""Scheduled sweep — build a periodic SCHEDULED run across enabled sources.

The scheduled trigger is the Container Apps cron job of the design (§8): on each
tick it scopes a run to *all currently enabled* source kinds and hands it to the
conveyor. ``sweep`` only *builds* the run (so callers can inspect/route it);
``run_sweep`` is the convenience that also drives it through ``run_line``.
...
__all__ = ["PAUSED_MESSAGE", "STATION", "run_sweep", "sweep"]
